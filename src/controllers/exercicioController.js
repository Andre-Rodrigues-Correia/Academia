...
export const updateExercicio = async (req, res, next) => {
  try {
    const updatedExercicio = await Exercicio.findByIdAndUpdate(
      req.params.id,
      { $set: req.body },
      { new: true },
    );
...
export const deleteExercicio = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ message: 'Exercicio excluida com sucesso' });
  } catch (error) {
    next(error);
  }
};
...
export const getExercicio = async (req, res, next) => {
  try {
    const exercicios = await Exercicio.findById(req.params.id);
